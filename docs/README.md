...
| [Log Message](./log-message/v1.1/docs/log-message.md) | Logging, Analytics & Monitoring, MCP, WebSub, WebBroker | Logs request/response payloads and headers. Supports inline (real-time) logging and an optional traffic-logging mode (`enableTrafficLogging`) that emits a latency-enriched line per API via the traffic-logging publisher. |
