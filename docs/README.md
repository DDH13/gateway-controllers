...
| [Log Message](./log-message/v1.1/docs/log-message.md) | Logging, Analytics & Monitoring, MCP, WebSub, WebBroker | Logs request/response payloads and headers inline in real time by default, or emits a single latency-enriched line per request in an optional stdout traffic-logging mode. |
